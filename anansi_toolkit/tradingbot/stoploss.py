...
class Treshold:
    def __init__(self, n_measurements: int, n_positives: int):
        ...
        self.n_positives = (
            n_positives if n_positives <= n_measurements else n_measurements
        )
    ...
    class DefaultParameters:
        def __init__(self):
            ...
            self.first_trigger = Trigger(
                rate=10, treshold=Treshold(n_measurements=5, n_positives=3)
            )
            self.second_trigger = Trigger(
                rate=3, treshold=Treshold(n_measurements=60, n_positives=20)
            )
            self.third_trigger = Trigger(
                rate=1, treshold=Treshold(n_measurements=120, n_positives=40)
            )
            self.update_target_if = Trigger(
                rate=0.7, treshold=Treshold(n_measurements=10, n_positives=3)
            )
