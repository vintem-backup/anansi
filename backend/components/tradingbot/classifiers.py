...
class DoubleCrossSMA(Classifier):
    ...
    def __init__(self, parameters, log):
        super(DoubleCrossSMA, self).__init__(parameters, log)
    ...
    def get_result_for_this(self, data):
        ...
        result.side = (
            SIDE.Long
            if result.SMA_smaller > result.SMA_larger
            else SIDE.Zeroed
        )
...
class TripleCrossSMA(Classifier):
    class DefaultParameters:
        def __init__(self):
            self.time_frame = "2h"
            self.price_metrics = "ohlc4"
            self.smaller_sample = 3
            self.middle_sample = 20
            self.larger_sample = 80
    
    def __init__(self, parameters, log):
        super(TripleCrossSMA, self).__init__(parameters, log)
    ...
    def get_result_for_this(self, data):
        ...
        result.SMA_middle = (
            data.apply_indicator.trend.simple_moving_average(
                number_of_candles=self.parameters.middle_sample,
                metrics=self.parameters.price_metrics,
            )
        ).last()
        ...
        result.side = (
            SIDE.Long
            if (
                (result.SMA_smaller > result.SMA_larger)
                and result.SMA_middle > result.SMA_larger
            )
            else SIDE.Zeroed
        )
        # data.KlinesDateTime.from_human_readable_to_timestamp()
